...
class alipayDmf{
     ...
     * 生成当面付订单信息(前台调用)
     * @param $money 订单金额
     * @param $OrderId 生成自定义订单号
     * @param $timeOut 该笔订单的过期时间
     * @return array
     * code        ：  返回代码
     * msg         :   返回消息
     ...
     * qr_code     ：  二维码
     */
    public function createDmfOrder($appid,$privateKey,$money,$OrdeTitle,$OrderId,$timeOut='2h')
    {
        // 配置支付宝应用ID  appid
        $this->setAppid($appid);
        // 配置 商户私钥 private_key
        ...
        // 配置生成自定义的商家不重复的订单号
        $this->setOutTradeNo($OrderId);
        // 配置生成的订单标题
        $this->setOrderName($OrdeTitle);
        // 配置订单过期时间
        $this->setTimeoutExpress($timeOut);
        // 开始生成订单
        $result = $this->doPay();
        // 返回生成的数组数据
        return  $result;
    ...
    /**
     * 查询订单情况(前台调用)
     * @param $appid 支付宝当面付应用id
     * @param $privateKey 商户私钥
     * @param $orderID 商户自定义订单
     * @return array 返回数组
     * 如果返回的code !=1000 要注意 要单独获取消息为 ['sub_msg'] 应该是交易不存在 。其他code=1000正常
     */
    public function queryDmfOrder($appid,$privateKey,$orderID)
    {
        // 配置支付宝应用ID  appid
        $this->setAppid($appid);
        // 配置 商户私钥 private_key
